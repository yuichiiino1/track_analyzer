bearing.mean.timecourse <- function(tracks=1:track.n, minturns=1, maxturns=Inf, 
          ...
          for(j in 1:length(which(temp))){
            ...
            if(pcount>=minturns && pcount<=maxturns){
              ...
              if(length(mainturn)>=1){
                ...
                dXPir[PirRun[[track.i]][startpoint:endpoint] == "R" & c(rep(F,plotprevtime-dummypoint),rep(T,(endpoint-startpoint)-(plotprevtime-dummypoint)+1))] <- NA
                dYPir <- dY[[track.i]][startpoint:endpoint]
                dYPir[PirRun[[track.i]][startpoint:endpoint] == "R" & c(rep(F,plotprevtime-dummypoint),rep(T,(endpoint-startpoint)-(plotprevtime-dummypoint)+1))] <- NA
  ...
  dev.new()
  fileheader2 <- paste0("mean_track_postturn_Pir",format(Sys.time(), "%Y%m%d_%H%M%S"))
