...
mainWindow <- function(){
  ...
  tbl[12,2] <- gcombobox(c("none","salt","odor"),container=tbl,handler=refresh) #source
  tbl[13,1] <- glabel("gradient",container=tbl)
  tbl[13,2] <- gcombobox(c("geometrical","numerical"),container=tbl,handler=refresh) #gradient
  tbl[14,1] <- glabel("plate_format",container=tbl)
  tbl[14,2] <- gcombobox(c("none","kunitomo","center-peak","2spot","12point","2odornew","1point"),container=tbl,handler=refresh) #plate_format
  tbl[15,1] <- glabel("datatype",container=tbl)
  tbl[15,2] <- gcombobox(c("multi","single","none"),container=tbl,editable=TRUE,handler=refresh) #datatype
  tbl[16,1] <- glabel("mark_left_to_right",container=tbl)
  tbl[16,2] <- gcombobox(c("TRUE","FALSE"),container=tbl, coerce.with=as.logical,editable=TRUE,handler=refresh) #mark_left_to_right
...
setdatafolder <- function(f){
  ...
  for(file.i in 1:length(tempfiles)){
...
read.files <- function(h,...){ #file.numbers=1:length(files), datatype){
  files <- strsplit(svalue(datafoldercontent, drop=TRUE)[[1]],"\n")[[1]]
  if(length(files)==0){
    files <- strsplit(svalue(datafoldercontent, drop=FALSE)[[1]],"\n")[[1]]
  }
  files[files == ""] <- NULL
  ...
  if(datatype == "multi")
  {
    ...
    for (file.i in file.numbers)
    {
      ...
      d0<<-read.table(paste(datadir,files[file.i],sep=""), header=F, comment.char=c("/"), fill=T, colClasses=c("character","numeric","numeric"))
      #5.1  Markpointsを読みとる
      lines <- readLines(paste(datadir,files[file.i],sep=""), n=10)
  ...
  if(datatype == "single")
  {
    ...
    for(file.i in file.numbers)
    {
      cat(paste(" ",file.i,sep=""))
      track.i <- track.i + 1
      d0<<-read.table(paste(datadir,files[file.i],sep=""), header=T, skip=2, fill=T, colClasses="numeric")
...
#関数adjust.position
co$adjust.position<-'
adjust.position(plates=1:plate.n)
...
'
adjust.position<-function(plates=1:plate.n){
  ...
  for(plate.i in plates)
  {
    if(plate_format=="2spot"){
      ...
      for(track.i in plate.track[[plate.i]])
      {
        U <- dX[[track.i]]-X0
        V <- dY[[track.i]]-Y0
        dX[[track.i]] <<- U*ct - V*st + 19.46
        dY[[track.i]] <<- V*ct + U*st + 50
        #browser()
      }
    }
    if(plate_format=="2odornew"){
    ...
    if(plate_format=="kunitomo"){
      ...
      for(track.i in plate.track[[plate.i]])
      {
        U <- dX[[track.i]]-X0
        V <- dY[[track.i]]-Y0
        dX[[track.i]] <<- U*ct + V*st + 50  #5.4β1：プラスマイナス逆だった
        dY[[track.i]] <<- V*ct - U*st + 50
        #browser()
      }
    }
    if(plate_format=="center-peak"){
      ...
      X0 <- mean(markpoints[[plate.i]][,1]); Y0 <- mean(markpoints[[plate.i]][,2]) # 平均の値を中心(50,50)とする
      for(track.i in plate.track[[plate.i]])
      {
        dX[[track.i]] <<- dX[[track.i]]-X0+50
        dY[[track.i]] <<- dY[[track.i]]-Y0+50
      }
    }
    }
  }
